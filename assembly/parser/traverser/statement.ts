...
function parseTryStatement(): void {
  ...
  if (match(tt._catch)) {
    ...
    let catchBindingStartTokenIndex = -1;
    if (match(tt.parenL)) {
      // state.scopeDepth++;
    ...
    if (catchBindingStartTokenIndex !== -1) {
...
export function parseBlock(isFunctionScope: boolean = false, /*contextId: i32 = 0*/): void {
  const startTokenIndex = state.tokens.length;
  // state.scopeDepth++;
  expect(tt.braceL);
  // if (contextId) {
...
export function parseFunction(
  ...
  let nameScopeStartTokenIndex = -1;
...
function parseClassMember(memberStart: i32, /*classContextId: number*/): void {
  // if (isTypeScriptEnabled) {
  if (true) {
...
function parseClassMethod(functionStart: i32, isConstructor: boolean): void {
  // if (isTypeScriptEnabled) {
  if (true) {
    tsTryParseTypeParameters();
  // } else if (isFlowEnabled) {
...
export function parseImport(): void {
  ...
  if (true && match(tt.name) && lookaheadType() === tt.eq) {
    tsParseImportEqualsDeclaration();
    return;
  }
  // if (isTypeScriptEnabled) {
